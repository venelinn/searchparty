...
export default async function MediaItemPage({ params }: Props) {
	...
	const fullGallery = [
		...videos.map((video: any) => ({
			...
			poster: mediaItem.videos.find((v: any) => v.id === video.id)?.thumb?.[0]?.src
				? getOptimizedImage(mediaItem.videos.find((v: any) => v.id === video.id).thumb[0], 1000).url
				: video.thumbnail,
			isVideo: true,
			width: video.thumbnailWidth,
			height: video.thumbnailHeight,
			title: mediaItem.videos.find((v: any) => v.id === video.id)?.videoTitle || video.title,
		})),
	...
	const thumbGallery = [
		...videos.map((video: any) => ({
			src: mediaItem.videos.find((v: any) => v.id === video.id)?.thumb?.[0]?.src
				? getOptimizedImage(mediaItem.videos.find((v: any) => v.id === video.id).thumb[0], 700).url
				: video.thumbnail,
			isVideo: true,
			width: video.thumbnailWidth,
			height: video.thumbnailHeight,
		...
		...mediaItem.images.map((img: any) => {
			...
			return { ...img, src: url, width, height, isVideo: false, title: img?.title || mediaItem.title };
		}),
	];
	...
	return (
		<Layout
			...
			page={{ title: mediaItem.title, locale: pageLocale }}
		>
			<Section
				contentAlign="center"
