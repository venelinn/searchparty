  ...
  ) =>
{
	...
	if (label && isExternal && externalHref) {
		return (
        <a className={classes} target="_blank" rel="noopener noreferrer" href={externalHref}>
          {renderContent()}
        </a>
      );
	}
	...
	if (label) {
		return (
        <button ref={ref} type={type} className={classes} onClick={onClick} disabled={disabled}>
          {renderContent()}
        </button>
      );
	}
