 ...
 *
 * Wrapped in React `cache()` (see the `getPageBySlug` export below) so the two
 * calls per request (generateMetadata + the page body) dedupe into one —
 * important in preview, where the data cache is bypassed and both calls would
 * otherwise hit Contentful.
 */
...
export async function getContentItem(
	...
	const res = await cachedContentful(
		...
		["item", contentType, id, contentfulLocale, String(!!preview)],
		{ bypass: !!preview },
	)
...
async function getContentModel(
	...
	try {
		const client = getClient(preview)
		const entries = await cachedContentful(
			() =>
				client.getEntries({
					content_type: contentType,
					locale: contentfulLocale,
			...
			["model", contentType, contentfulLocale, String(!!preview)],
			{ bypass: !!preview },
		)
